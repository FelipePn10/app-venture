...
const TYPE_MRP = ["MRP", "MIN_MAX", "KANBAN", "REORDER_POINT", "MPS"];
const ENG_TYPES = ["RAW_MATERIAL", "MANUFACTURED", "FINISHED", "DE_TERCEIRO"];
const STRUCTS = ["SIMPLE", "ASSEMBLY", "INDUSTRIAL"];

const EMPTY_NEW = {
  code: "", nature: 0, group_code: "", modifier_code: "",
  uom: "UN", minimum_stock: "0", eng_type: "MANUFACTURED", type_struct: "ASSEMBLY",
  weight_net: "0", type_mrp: "MRP", llc: "2", type_of_use: "INDUSTRIALIZATION",
};
...
export function Vitm0100Page(): JSX.Element {
  ...
  const criar = () => run(async () => {
    if (!nf.group_code || !nf.modifier_code) { setFeedback({ type: "error", message: "Grupo e modificador (PDM) são obrigatórios." }); return; }
    const dto = {
      ...(nf.code ? { code: Number(nf.code) } : {}),
      nature: Number(nf.nature),
      pdm: { group_code: Number(nf.group_code), modifier_code: Number(nf.modifier_code), attributes: [] },
      warehouse: { warehouse_code: 1, unit_of_measurement: nf.uom, minimum_stock: Number(nf.minimum_stock) || 0 },
    ...
    <div className="fsc-root">
      ...
      <div className="fsc-body">
        ...
        <div className="fsc-card"><div className="fsc-card-body"><div className="fsc-grid">
          ...
          <div className="fsc-field fsc-col-2"><label className="fsc-label">Tipo eng.</label><select className="fsc-input" value={nf.eng_type} onChange={(e) => setNf((p) => ({ ...p, eng_type: e.target.value }))}>{ENG_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
          <div className="fsc-field fsc-col-2"><label className="fsc-label">Estrutura</label><select className="fsc-input" value={nf.type_struct} onChange={(e) => setNf((p) => ({ ...p, type_struct: e.target.value }))}>{STRUCTS.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
          <div className="fsc-field fsc-col-2"><label className="fsc-label">Tipo MRP</label><select className="fsc-input" value={nf.type_mrp} onChange={(e) => setNf((p) => ({ ...p, type_mrp: e.target.value }))}>{TYPE_MRP.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
