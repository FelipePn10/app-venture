...
const ANALISES = ["Cliente", "Grupo-Motivo", "Representante"];
const QUEBRAS = ["Sem Quebra", "Por Empresa", "Por Grupo", "Por Tipo Chamado", "Por Situação"];
const OPCOES_RELATORIO = ["Completo", "Resumido"];
const UFS = ["SP", "RJ", "MG", "PR", "RS", "BA", "SC", "DF", "GO", "CE"];
...
const configInicial: ConfiguracaoRelatorio = {
  analise: "Cliente",
  quebra: "Sem Quebra",
  opcoes: "Completo",
  imprime_selecao: false,
};
...
export function Vatc0380Page(): JSX.Element {
  ...
  const [config, setConfig]     = useState<ConfiguracaoRelatorio>(configInicial);
  const [feedback, setFeedback] = useState<FeedbackState>(null);
  const [isGenerating, setIsGenerating] = useState(false);

  const setFilter = useCallback(<K extends keyof FiltrosRelatorio>(key: K, value: FiltrosRelatorio[K]) => {
    setFiltros((prev) => ({ ...prev, [key]: value }));
  }, []);

  const setConf = useCallback(<K extends keyof ConfiguracaoRelatorio>(key: K, value: ConfiguracaoRelatorio[K]) => {
    setConfig((prev) => ({ ...prev, [key]: value }));
  }, []);

  const isVistoria = filtros.situacao === "Vistoria";
  const isPosicaoPendente = filtros.posicao === "Pendente";
  ...
  async function handleGerarRelatorio() {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: `Relatório de chamados gerado com sucesso. Análise por ${config.analise}, formato ${config.opcoes}.`,
      });
    } catch (error) {
      setFeedback({
        type: "error",
        message: normalizeError(error, "Erro ao gerar relatório."),
      });
    } finally {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .atc-select:disabled { background: #f0f4ee; color: #8aaa94; cursor: not-allowed; border-color: #e0ead8; }

        .atc-field-hint { font-size: 11px; color: #7a9c84; margin-top: 2px; line-height: 1.45; }

        /* ── TOGGLE ── */
        .atc-toggle-row { display: flex; align-items: center; gap: 10px; }
      ...
      <div className="atc-root">
        ...
        <header className="atc-topbar">
          <div className="atc-topbar-left">
            ...
            <span className="atc-screen-title">VATC0380 — Relatório de Chamados</span>
          </div>
        </header>
        ...
        <div className="atc-actionbar">
          <div className="atc-action-group">
            <span className="atc-action-label">Relatório</span>
            <button
            ...
            >
              {isGenerating
                ? <><div className="atc-spinner" />Gerando...</>
                : <>
                    <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                      <path d="M3 2h10v12H3z" stroke="currentColor" strokeWidth="1.4" />
                      <path d="M6 2v12M10 2v12M3 6h10M3 10h10" stroke="currentColor" strokeWidth="1" />
                    </svg>
                    Gerar Relatório
                  </>
              }
        ...
        <div className="atc-body">
          ...
          <div className="atc-section-banner">
            ...
            <span className="atc-section-banner-hint">Defina os filtros para o relatório</span>
          </div>
          ...
          <div className="atc-card">
            <div className="atc-card-header">
              <div className="atc-card-header-left">
                ...
                <span className="atc-card-title">Filtros do Relatório</span>
              </div>
            </div>
            ...
            <div className="atc-card-body">
              ...
              <div className="atc-grid" style={{ marginBottom: 16 }}>
                <div className="atc-field atc-col-2">
                  ...
                  <input
                    ...
                    value={filtros.chamado}
                    onChange={(e) => setFilter("chamado", e.target.value)}
                  />
                </div>
                <div className="atc-field atc-col-2">
                ...
                <div className="atc-field atc-col-2">
                  <label className="atc-label">UF</label>
                  <select
              ...
              <div className="atc-grid" style={{ marginBottom: 16 }}>
                <div className="atc-field atc-col-2">
                  <label className="atc-label">Cidade</label>
                  <input
                    className="atc-input"
          ...
          <div className="atc-section-banner">
            <span className="atc-section-banner-pill">2 — Configuração</span>
            <div className="atc-section-banner-line" />
            <span className="atc-section-banner-hint">Configure a saída do relatório</span>
          </div>
          ...
          <div className="atc-card">
            <div className="atc-card-header">
              <div className="atc-card-header-left">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                  <path d="M3 2h10v12l-5-3-5 3V2z" stroke="#3e9654" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <span className="atc-card-title">Configuração do Relatório</span>
              </div>
            </div>

            <div className="atc-card-body">
              <div className="atc-grid">
                {/* Análise — disabled when Posição === Pendentes */}
                <div className="atc-field atc-col-3">
                  <label className="atc-label">Análise</label>
                  <select
                    className="atc-select"
                    value={config.analise}
                    onChange={(e) => setConf("analise", e.target.value)}
                    disabled={isPosicaoPendente}
                  >
                    {ANALISES.map((a) => (
                  ...
                  {isPosicaoPendente && (
                    <span className="atc-field-hint">Análise indisponível quando Posição = "Pendente".</span>
                  )}
                </div>

                {/* Quebra do Relatório */}
                <div className="atc-field atc-col-3">
                  <label className="atc-label">Quebra do Relatório</label>
                  <select
                    className="atc-select"
                    value={config.quebra}
                    onChange={(e) => setConf("quebra", e.target.value)}
                  >
                    {QUEBRAS.map((q) => (
                ...
                {/* Opções */}
                <div className="atc-field atc-col-2">
                  <label className="atc-label">Opções</label>
                  <select
                    className="atc-select"
                    value={config.opcoes}
                    onChange={(e) => setConf("opcoes", e.target.value)}
                  >
                    {OPCOES_RELATORIO.map((o) => (
                      <option key={o} value={o}>{o}</option>
                    ))}
                  </select>
                </div>

                {/* Imprime Opções de Seleção */}
                <div className="atc-field atc-col-4">
                  <label className="atc-label">Imprime Opções de Seleção</label>
                  <div style={{ paddingTop: 6 }}>
                    <div className="atc-toggle-row">
                      <label className="atc-toggle">
                        <input
                          type="checkbox"
                          checked={config.imprime_selecao}
                          onChange={(e) => setConf("imprime_selecao", e.target.checked)}
                        />
                        <div className="atc-toggle-track" />
                        <div className="atc-toggle-thumb" />
                      </label>
                      <span className="atc-toggle-label">
                        {config.imprime_selecao ? "Sim" : "Não"}
                      </span>
                    </div>
                  </div>
                  <span className="atc-field-hint">Inclui os critérios de seleção no rodapé do relatório.</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <footer className="atc-footer">
          <div className="atc-footer-left">
            <div className="atc-footer-stat">
              Análise: <strong>{isPosicaoPendente ? "—" : config.analise}</strong>
            </div>
            <div className="atc-footer-stat">
              Quebra: <strong>{config.quebra}</strong>
            </div>
            <div className="atc-footer-stat">
              Opções: <strong>{config.opcoes}</strong>
            </div>
            <div className="atc-footer-stat">
              Imprime Seleção: <strong>{config.imprime_selecao ? "Sim" : "Não"}</strong>
            </div>
          </div>
          <div className="atc-footer-stat" style={{ gap: 8 }}>
            <span style={{ color: "#b0c8b8", fontSize: 11 }}>
              Filtros: {[
                filtros.chamado && `Chamado: ${filtros.chamado}`,
                filtros.posicao && `Posição: ${filtros.posicao}`,
                filtros.situacao && `Situação: ${filtros.situacao}`,
