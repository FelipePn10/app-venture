                  ...
                  {folder === "dados" && (
                    <div className="erp-fieldset">
                      ...
                      <div className="erp-fieldset-body">
                        ...
                        <div className="erp-field erp-c2"><label className="erp-label">Pessoa</label><select className="erp-input" value={form.person_type} onChange={(e) => setF("person_type", e.target.value as PersonType)}>{PERSON_TYPES.map((x) => <option key={x} value={x}>{enumLabel(x)}</option>)}</select></div>
                        <div className="erp-field erp-c2"><label className="erp-label">Tipo doc.</label><select className="erp-input" value={form.document_type} onChange={(e) => setF("document_type", e.target.value as DocumentType)}>{DOCUMENT_TYPES.map((x) => <option key={x} value={x}>{enumLabel(x)}</option>)}</select></div>
                        ...
                        <div className="erp-field erp-c3"><label className="erp-label">Contrib. ICMS</label><select className="erp-input" value={form.icms_contributor} onChange={(e) => setF("icms_contributor", e.target.value as IcmsContributor)}>{ICMS_CONTRIBUTORS.map((x) => <option key={x} value={x}>{enumLabel(x)}</option>)}</select></div>
                        <div className="erp-field erp-c2"><label className="erp-label">Obr. vitícola</label><select className="erp-input" value={form.viticola_obligation} onChange={(e) => setF("viticola_obligation", e.target.value as ViticolaObligation)}>{VITICOLA.map((x) => <option key={x} value={x}>{enumLabel(x)}</option>)}</select></div>
                        <div className="erp-field erp-c3"><label className="erp-label">Plataforma rastreio</label><select className="erp-input" value={form.tracking_platform} onChange={(e) => setF("tracking_platform", e.target.value as TrackingPlatform)}>{TRACKING_PLATFORMS.map((x) => <option key={x} value={x}>{enumLabel(x)}</option>)}</select></div>
