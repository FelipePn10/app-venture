    ...
    <div className="erp-screen">
      ...
      <div className="erp-toolbar">
        <div className="erp-tgroup">
          ...
          <div style={{ width: 230 }}><LookupField value={planCode ? Number(planCode) : undefined} loader={loadCrpPlans} entityLabel="plano MRP" onChange={(code) => setPlanCode(code ? String(code) : "")} /></div>
        ...
        <div className="erp-tgroup">
          ...
          <ExportButton title="VPRO0200 — CRP (Capacity Requirements Planning)" filename="vpro0200" build={() => ({
            columns: ["Centro", "Data", "Necessário (h)", "Disponível (h)", "Carga %", "Sobrecarga"],
            rows: rows.map((r) => [String(r.work_center_id), r.req_date?.slice(0, 10) ?? "—", num(r.required_hours), num(r.available_hours), num(r.load_pct), r.is_overloaded ? "Sim" : "Não"]),
            subtitle: summary ? `Plano ${summary.plan_code} · ${summary.total_entries} registro(s)` : undefined,
          })} />
        </div>
      </div>
        ...
        <div className="erp-fieldset">
          ...
          {tab === "centro" && (
            <div className="erp-fieldset-body">
                ...
                <div className="erp-field erp-c3"><label className="erp-label">Centro</label><LookupField value={wcId ? Number(wcId) : undefined} loader={loadWorkCenters} entityLabel="centro de trabalho" onChange={(code) => setWcId(code ? String(code) : "")} /></div>
