...
const list = (path: string, fields: RoutineField[] = [], query: string[] = [], label = "Consultar"): RoutineOperation => ({ label, method: "GET", path, fields, query });
const create = (path: string, example: string, adminOnly = false, label = "Cadastrar"): RoutineOperation => ({ label, method: "POST", path, fields: [json(example)], adminOnly });
const remove = (path: string, fields: RoutineField[] = [id()], adminOnly = false, label = "Desativar / excluir"): RoutineOperation => ({ label, method: "DELETE", path, fields, destructive: true, adminOnly });
...
const enumOpt = (name: string, label: string, options: string[]): RoutineField => ({ name, label, options });
...
export const OPERATIONAL_ROUTINES: Record<string, OperationalRoutine> = {
  VENT0100: routine("VENT0100", "Consulta de pedidos de venda", "Consulta pedidos e seus itens sem expor comandos de manutenção; use VPDV0200 quando for necessário alterar o pedido.", [
    ...
    { label: "Consultar itens", method: "GET", path: "/api/sales-order/items/{code}", fields: [id("code", "Pedido")] },
  ...
  VPDV0253: routine("VPDV0253", "Console de acompanhamento de pedidos", "Apresenta carteira, posição, itens, promessas e reprogramações para acompanhamento; não substitui o cadastro VPDV0200.", [
    list("/api/sales-order/list"),
    { label: "Pedidos faturados", method: "GET", path: "/api/sales-order/status/F", fields: [] },
    { label: "Abrir posição", method: "GET", path: "/api/sales-order/{code}", fields: [id("code", "Pedido")] },
    { label: "Itens e saldos", method: "GET", path: "/api/sales-order/items/{code}", fields: [id("code", "Pedido")] },
    { label: "Reprogramações", method: "GET", path: "/api/delivery-reschedule/list/{code}", fields: [id("code", "Pedido")] },
  ]),
  ...
  VENT0204: routine("VENT0204", "Cadastro de Grupo PDM", "Consulta, cadastra e altera famílias PDM vinculadas à empresa, usando o cadastro estrutural persistido no backend. Empresa e usuário responsável são definidos automaticamente pelo acesso atual.", [
    ...
    create("/api/pdm/create-group", '{"code":10,"description":"Família de produtos"}'),
    { label: "Alterar grupo", method: "PUT", path: "/api/pdm/groups/{code}", fields: [id("code", "Código do grupo"), json('{"description":"Família de produtos atualizada"}')] },
  ]),
  ...
  VSUP0650: routine("VSUP0650", "Histórico de movimentos de compra", "Consolida movimentos por fornecedor e item para rastreabilidade e análise de compras.", [
    list("/api/procurement/purchase-movements", [{ name: "supplier_code", label: "Fornecedor", type: "number" }, { name: "item_code", label: "Item" }, { name: "limit", label: "Limite", type: "number", defaultValue: "100" }], ["supplier_code", "item_code", "limit"]),
  ]),
  VSUP0600: routine("VSUP0600", "Inspeção de recebimento", "Cria roteiros, gera ordens, registra resultados, analisa e destina o material inspecionado.", [
    create("/api/procurement/receiving-inspection-routes", '{"enterprise_code":1,"basis":"ITEM","item_code":100,"mask":"","inspection_warehouse_id":2,"valid_from":"2026-07-14","steps":[{"sequence":1,"inspection_name":"Dimensão","kind":"NUMERIC","appointment_mode":"SAMPLE","is_required":true,"emits_label":false,"sample_qty":5,"acceptance_qty":5,"rejection_qty":1,"min_value":9.9,"max_value":10.1,"attributes":[]}]}', false, "Cadastrar roteiro de inspeção"),
    ...
    { label: "Analisar ordem", method: "POST", path: "/api/procurement/receiving-inspection-orders/{id}/analysis", fields: [id(), json('{"conform_qty":18,"rejected_qty":2,"rework_qty":0,"restricted_qty":0,"treatment":"ACCEPT_WITH_RESTRICTION","affects_supplier_score":true,"move_stock":true,"destination_warehouse_id":1,"rejection_warehouse_id":3,"notes":"Conferência concluída"}')] },
  ...
  VAVF0300: routine("VAVF0300", "Scorecard e IQF do fornecedor", "Consulta o histórico e calcula ou registra os indicadores de qualidade, entrega, comercial e atendimento.", [
    ...
    create("/api/procurement/supplier-scorecards/compute", '{"supplier_code":10,"period_start":"2026-01-01","period_end":"2026-06-30","commercial_score":100,"service_score":100,"persist":true,"notes":"Apuração semestral"}', false, "Calcular avaliação"),
  ...
  VCFG0100: routine("VCFG0100", "Conjuntos e variáveis do configurador", "Mantém conjuntos de respostas, variáveis, composição da máscara e traduções.", [
    ...
    remove("/api/configurator/variables/{varId}", [id("varId", "Variável")], false, "Excluir variável"),
  ...
  VCFG0500: routine("VCFG0500", "Descrições configuradas", "Mantém tipos, linhas e renderização das descrições de itens configurados.", [
    ...
    create("/api/configurator/item-descriptions", '{"item_code":100,"description_type_id":1}', false, "Cadastrar descrição do item"), list("/api/configurator/items/{itemCode}/descriptions", [id("itemCode", "Item")]),
  ...
  VTER0100: routine("VTER0100", "Preços de serviços de terceiros", "Mantém preços, vigências, regras, fórmula, custo resolvido, reajuste e histórico.", [
    list("/api/third-party-services/prices", [{ name: "item_from", label: "Item inicial" }, { name: "item_to", label: "Item final" }, { name: "supplier_from", label: "Fornecedor inicial", type: "number" }, { name: "supplier_to", label: "Fornecedor final", type: "number" }, { name: "operation_id", label: "Operação", type: "number" }, { name: "reference_date", label: "Data de referência", type: "date" }, { name: "price_type", label: "Tipo de preço" }, { name: "preferred", label: "Somente preferencial", type: "checkbox" }], ["item_from", "item_to", "supplier_from", "supplier_to", "operation_id", "reference_date", "price_type", "preferred"]), create("/api/third-party-services/prices", '{"item_code":"TEA452-0","supplier_code":10,"operation_id":1,"uom":"UN","reference_date":"2026-07-14T00:00:00Z","preferred":true,"unit_price":"25.50","freight_type":"FIXED","freight_value":"0","tax_percent":"0","reason":"Tabela 2026","rules":[]}', true),
    { label: "Abrir preço", method: "GET", path: "/api/third-party-services/prices/{id}", fields: [id()] },
    { label: "Alterar preço", method: "PUT", path: "/api/third-party-services/prices/{id}", adminOnly: true, fields: [id(), json('{"item_code":100,"supplier_code":10,"operation_id":1,"uom":"UN","reference_date":"2026-07-14T00:00:00Z","preferred":true,"unit_price":"25.50","freight_type":"FIXED","freight_value":"0","tax_percent":"0","reason":"Revisão","rules":[]}')] },
    list("/api/third-party-services/prices/resolve", [{ name: "item_code", label: "Item", required: true }, { name: "mask", label: "Máscara" }, { name: "supplier_code", label: "Fornecedor", type: "number" }, { name: "operation_id", label: "Operação", type: "number", required: true }, { name: "reference_date", label: "Data de referência", type: "date" }, { name: "attributes", label: "Atributos" }], ["item_code", "mask", "supplier_code", "operation_id", "reference_date", "attributes"], "Resolver preço do item"),
    list("/api/third-party-services/cost", [{ name: "item_code", label: "Item", required: true }, { name: "mask", label: "Máscara" }, { name: "operation_id", label: "Operação", type: "number", required: true }, { name: "reference_date", label: "Data de referência", type: "date" }, { name: "mode", label: "Modo", defaultValue: "STANDARD" }], ["item_code", "mask", "operation_id", "reference_date", "mode"], "Consultar custo do serviço"),
  ...
  VAPS0400: routine("VAPS0400", "Perfil de operadores APS", "Mantém contatos, funções, centro de custo, supervisão e limite de crédito do funcionário.", [
    ...
    { label: "Desativar contato", method: "DELETE", path: "/api/aps/employees/{employeeID}/contacts/{contactID}", fields: [id("employeeID", "Funcionário"), id("contactID", "Contato")], destructive: true, adminOnly: true },
    ...
    { label: "Desativar função", method: "DELETE", path: "/api/aps/employees/{employeeID}/functions/{functionID}", fields: [id("employeeID", "Funcionário"), id("functionID", "Função")], destructive: true, adminOnly: true },
  ]),
  VAPS0500: routine("VAPS0500", "Perfil industrial de máquinas", "Mantém uso, preparação, marca, responsáveis, serviços preventivos, itens e campos especiais.", [
    ...
    { label: "Desativar campo especial", method: "DELETE", path: "/api/aps/resources/{machineID}/special-values/{fieldID}", fields: [id("machineID", "Máquina"), id("fieldID", "Campo")], destructive: true, adminOnly: true },
  ]),
  ...
  VENG0400: routine("VENG0400", "Desenhos e revisões", "Mantém desenho técnico, revisões, distribuição controlada, características e parâmetros de fabricação.", [
    ...
    remove("/api/drawings/revisions/{revId}", [id("revId", "Revisão")], false, "Excluir revisão"),
    ...
    remove("/api/drawings/distributions/{distId}", [id("distId", "Distribuição")], false, "Excluir distribuição"),
    ...
    remove("/api/drawings/characteristics/{charLinkId}", [id("charLinkId", "Vínculo")], false, "Excluir vínculo de característica"),
  ...
  VEST0300: routine("VEST0300", "Máscaras de lote e série", "Define partes fixas, datas e sequências e gera códigos de lote pelo contexto do item.", [
    list("/api/lot-masks/"),
    { label: "Cadastrar máscara", method: "POST", path: "/api/lot-masks/", fields: [json('{"application":"SUPRIMENTOS","description":"Lote padrão","classification_type":"ITEM","zero_on_year_change":true}')] },
    { label: "Abrir máscara", method: "GET", path: "/api/lot-masks/{id}", fields: [id()] },
    { label: "Alterar máscara", method: "PUT", path: "/api/lot-masks/{id}", fields: [id(), json('{"application":"SUPRIMENTOS","description":"Lote padrão","classification_type":"ITEM","zero_on_year_change":true}')] },
    { label: "Adicionar parte", method: "POST", path: "/api/lot-masks/{id}/parts", fields: [id(), json('{"sequence":1,"part_type":"DATA","value":"","size":8,"date_format":"DDMMYYYY","zero_on_year_change":false}')] },
    { label: "Alterar parte", method: "PUT", path: "/api/lot-masks/parts/{partId}", fields: [id("partId", "Parte"), json('{"sequence":1,"part_type":"DATA","value":"","size":8,"date_format":"DDMMYYYY","zero_on_year_change":false}')] },
    { label: "Desativar parte", method: "DELETE", path: "/api/lot-masks/parts/{partId}", fields: [id("partId", "Parte")], destructive: true },
    { label: "Gerar código de lote", method: "POST", path: "/api/lot-masks/generate", fields: [json('{"application":"SUPRIMENTOS","item_code":"100"}')] },
    { label: "Desativar máscara", method: "DELETE", path: "/api/lot-masks/{id}", fields: [id()], destructive: true },
  ]),
  VSUP0630: routine("VSUP0630", "Tolerâncias de pedido de compra", "Define tolerâncias por faixa e fornecedor e avalia quantidade, preço ou prazo recebido.", [
    list("/api/purchase-order-tolerances/", [], [], "Consultar tolerâncias"), create("/api/purchase-order-tolerances/", '{"tolerance_type":"QUANTITY","applies_to":"RECEIPT","interval_min":"0","interval_max":"1000","tolerance_value":"5","value_type":"PERCENT","action":"WARN","is_active":true}'),
    ...
    create("/api/purchase-order-tolerances/evaluate", '{"tolerance_type":"QUANTITY","applies_to":"RECEIPT","supplier_code":10,"expected":"100","actual":"104"}', false, "Avaliar tolerância"), remove("/api/purchase-order-tolerances/{id}"),
  ]),
  ...
  VEXP0120: routine("VEXP0120", "Instruções e caixas de despacho", "Mantém orientações de entrega e posições físicas usadas na montagem das cargas.", [
    { label: "Consultar instruções de entrega", method: "GET", path: "/api/shipments/delivery-instructions", fields: [{ name: "load_code", label: "Carga", type: "number" }, { name: "active_only", label: "Somente ativas", type: "checkbox", defaultValue: true }], query: ["load_code", "active_only"] },
    { label: "Cadastrar instrução de entrega", method: "POST", path: "/api/shipments/delivery-instructions", fields: [json('{"load_code":100,"customer_id":200,"title":"Agendamento obrigatório","instruction":"Entregar somente após confirmação do cliente.","priority":1}')] },
    { label: "Consultar caixas de despacho", method: "GET", path: "/api/shipments/dispatch-boxes", fields: [{ name: "active_only", label: "Somente ativas", type: "checkbox", defaultValue: true }], query: ["active_only"] },
    { label: "Cadastrar caixa de despacho", method: "POST", path: "/api/shipments/dispatch-boxes", fields: [json('{"code":"BOX-01","description":"Doca principal","warehouse_id":1,"zone":"EXPEDICAO"}')] },
  ]),
  ...
  VCLI0202: routine("VCLI0202", "Políticas de frete e formação de preço", "Mantém o percentual de frete e demais componentes da política comercial aplicada à formação do preço.", [
    { label: "Listar políticas", method: "GET", path: "/api/customers/support/sales-price-policies/" },
    { label: "Criar política", method: "POST", path: "/api/customers/support/sales-price-policies/", fields: [json('{"description":"Política padrão","cost_source":"STANDARD_TOTAL","priority":1,"sequence":1,"policy_scope":"PREC","policy_types":"FREIGHT","markup_pct":0,"margin_pct":0,"max_margin_pct":0,"ideal_margin_pct":0,"margin_step_pct":0,"expenses_pct":0,"taxes_pct":0,"freight_pct":5,"commission_pct":0,"discount_pct":0,"min_margin_pct":0,"max_discount_pct":0,"incidences_json":{},"observation":"Frete comercial"}')] },
    { label: "Abrir política", method: "GET", path: "/api/customers/support/sales-price-policies/{code}", fields: [id("code", "Política comercial")] },
    { label: "Alterar política", method: "PUT", path: "/api/customers/support/sales-price-policies/{code}", fields: [id("code", "Política comercial"), json('{"description":"Política padrão","cost_source":"STANDARD_TOTAL","priority":1,"sequence":1,"policy_scope":"PREC","policy_types":"FREIGHT","markup_pct":0,"margin_pct":0,"max_margin_pct":0,"ideal_margin_pct":0,"margin_step_pct":0,"expenses_pct":0,"taxes_pct":0,"freight_pct":5,"commission_pct":0,"discount_pct":0,"min_margin_pct":0,"max_discount_pct":0,"incidences_json":{},"observation":"Frete comercial"}')] },
  ]),
  ...
  VSUP0660: routine("VSUP0660", "Parâmetros e contatos complementares do fornecedor", "Configura regras corporativas do cadastro de fornecedores e inclui telefones e e-mails em contatos já persistidos.", [
    ...
    create("/api/suppliers/contacts/phones", '{"contact_id":1,"value":"11999999999","ranking":1}', false, "Cadastrar telefone do contato"),
    create("/api/suppliers/contacts/emails", '{"contact_id":1,"value":"compras@fornecedor.com.br","ranking":1}', false, "Cadastrar e-mail do contato"),
  ]),
  VSUP0670: routine("VSUP0670", "Itens do fornecedor e relatórios de qualidade", "Consulta os itens vinculados ao fornecedor e mantém evidências documentais de qualidade do vínculo item-fornecedor.", [
    ...
    { label: "Buscar pelo código ou descrição do fornecedor", method: "GET", path: "/api/item-suppliers/search", fields: [id("supplier_code", "Fornecedor"), { name: "term", label: "Código ou descrição externa", required: true }], query: ["supplier_code", "term"] },
    ...
    { label: "Relatórios da inspeção", method: "GET", path: "/api/procurement/receiving-inspection-orders/{id}/quality-reports", fields: [id("id", "Ordem de inspeção")] },
    { label: "Associar relatório à inspeção", method: "POST", path: "/api/procurement/receiving-inspection-orders/{id}/quality-reports", fields: [id("id", "Ordem de inspeção"), { name: "quality_report_id", label: "Relatório de qualidade", type: "number", required: true }] },
    { label: "Remover relatório da inspeção", method: "DELETE", path: "/api/procurement/receiving-inspection-orders/{id}/quality-reports/{reportID}", fields: [id("id", "Ordem de inspeção"), id("reportID", "Relatório de qualidade")], destructive: true },
  ]),
  VSUP0680: routine("VSUP0680", "Fontes e atualização de preços de compra", "Consulta candidatos e preços originados de compras, aplica seleções na tabela e copia ajustes entre seus itens.", [
    ...
    create("/api/purchase-price-tables/sources/apply", '{"table_code":1,"overwrite":false,"selections":[{"source_type":"PURCHASE_ORDER","source_id":100}]}', false, "Aplicar fonte de preço"),
    create("/api/purchase-price-tables/items/copy-adjustments", '{"source_item_id":1,"target_item_id":2,"mode":"COPY"}', false, "Copiar e reajustar itens"),
  ]),
  ...
  VGAR0211: routine("VGAR0211", "Devoluções de atendimento e garantia", "Registra e consulta retornos associados ao chamado real do consumidor.", [
    { label: "Listar RMAs do chamado", method: "GET", path: "/api/technical-assistance/calls/{code}/rmas", fields: [id("code", "Chamado de assistência")] },
    { label: "Abrir RMA", method: "GET", path: "/api/technical-assistance/rmas/{rmaCode}", fields: [id("rmaCode", "RMA")] },
    { label: "Solicitar RMA", method: "POST", path: "/api/technical-assistance/calls/{code}/rmas", idempotent: true, fields: [id("code", "Chamado de assistência"), json('{"reason_code":"DEVOLUCAO","reason_description":"Solicitação do cliente","eligibility_status":"ELEGIVEL","eligibility_reason":"Produto dentro das regras de garantia","sla_due_at":"2026-09-15T18:00:00Z","product_cost":0,"freight_cost":0,"service_cost":0,"items":[{"call_item_code":1,"item_code":100,"quantity":1,"serial_number":"","lot_number":"","requested_destination":"REPARO"}]}')] },
    { label: "Avançar RMA", method: "PATCH", path: "/api/technical-assistance/rmas/{rmaCode}/transition", fields: [id("rmaCode", "RMA"), json('{"status":"AUTORIZADO","reason":"Devolução autorizada","correlation_id":"operacao-rma","authorization_number":"RMA-2026-001","reverse_carrier_code":null,"reverse_tracking_code":null,"inspection_notes":null,"destination":null,"fiscal_document_key":null,"stock_movement_code":null}')] },
    { label: "Listar evidências do RMA", method: "GET", path: "/api/technical-assistance/rmas/{rmaCode}/evidences", fields: [id("rmaCode", "RMA")] },
    { label: "Enviar evidência do RMA", method: "POST", path: "/api/technical-assistance/rmas/{rmaCode}/evidences", fields: [id("rmaCode", "RMA"), { name: "file", label: "Arquivo PDF, PNG ou JPEG", type: "file-upload", required: true, accept: ".pdf,.png,.jpg,.jpeg,application/pdf,image/png,image/jpeg" }] },
    { label: "Baixar evidência do RMA", method: "GET", path: "/api/technical-assistance/rmas/{rmaCode}/evidences/{evidenceID}", downloadFilename: "evidencia-rma", fields: [id("rmaCode", "RMA"), { name: "evidenceID", label: "Identificador da evidência", required: true }] },
    { label: "Abrir chamado", method: "GET", path: "/api/consumer-service/calls/{code}", fields: [id("code", "Chamado do consumidor")] },
    { label: "Listar anexos do chamado", method: "GET", path: "/api/consumer-service/calls/{code}/attachments", fields: [id("code", "Chamado do consumidor")] },
    { label: "Baixar anexo", method: "GET", path: "/api/consumer-service/calls/{code}/attachments/{attachmentCode}/download", downloadFilename: "anexo-do-chamado", fields: [id("code", "Chamado do consumidor"), id("attachmentCode", "Código do anexo")] },
    { label: "Excluir anexo", method: "DELETE", path: "/api/consumer-service/calls/{code}/attachments/{attachmentCode}", destructive: true, fields: [id("code", "Chamado do consumidor"), id("attachmentCode", "Código do anexo")] },
    { label: "Registrar retorno", method: "POST", path: "/api/consumer-service/calls/{code}/returns", fields: [id("code", "Chamado do consumidor"), json('{"contacted_at":"2026-07-14T10:00:00Z","contact_type":"PHONE","description":"Cliente orientado sobre a devolução.","next_return_at":"2026-07-15T10:00:00Z","user_code":1}')] },
  ]),
  VENG0500: routine("VENG0500", "Consulta e manutenção avançada de estruturas", "Altera componentes e consulta filhos, explosão e onde-usado da estrutura do item.", [
    ...
    { label: "Consultar estrutura", method: "GET", path: "/api/items/structure/consult", fields: [{ name: "item_code", label: "Item", required: true }, { name: "mask", label: "Máscara" }, { name: "effectiveness_date", label: "Efetividade", type: "date" }, { name: "levels", label: "Níveis (0 = todos)", type: "number", defaultValue: "0" }], query: ["item_code", "mask", "effectiveness_date", "levels"] },
  ...
  VPRO1100: routine("VPRO1100", "Parâmetros de estoque da manufatura", "Configura lotes, baixa automática, controle por item e endereçamento de almoxarifados da produção.", [
    { label: "Parâmetros gerais", method: "PUT", path: "/api/production-order/manufacturing-stock-settings", fields: [json('{"lot_return_mode":"A","auto_issue_lots":false,"movement_from":"2026-07-01T00:00:00Z","movement_to":"2026-07-31T23:59:59Z"}')] },
  ...
  VVND0600: routine("VVND0600", "Análise, atendimento e conferência de pedidos", "Executa as decisões pós-cadastro que controlam análise, atendimento, conferência e atraso do pedido de venda.", [
    {
      label: "Consultar pedidos (filtros)",
      method: "GET",
      path: "/api/sales-order/search",
      fields: [
        enumOpt("status", "Situação", ["R", "P", "A", "OA", "OF", "F", "CANCELLED"]),
        enumOpt("commercial_analysis_status", "Análise comercial", ["NOT_ANALYZED", "APPROVED", "REJECTED"]),
        enumOpt("financial_analysis_status", "Análise financeira", ["NOT_ANALYZED", "APPROVED", "REJECTED"]),
        enumOpt("conference_status", "Conferência", ["PENDING", "CONFERRED", "DIVERGENT"]),
        enumOpt("release_status", "Liberação", ["BLOCKED", "MANUAL_RELEASED", "RELEASED"]),
        { name: "customer_code", label: "Cliente", type: "number" },
        { name: "representative_code", label: "Representante", type: "number" },
        { name: "item_code", label: "Item" },
        { name: "emission_from", label: "Emissão de", type: "date" },
        { name: "emission_to", label: "Emissão até", type: "date" },
        { name: "limit", label: "Limite", type: "number", defaultValue: "500" },
      ],
      query: ["status", "commercial_analysis_status", "financial_analysis_status", "conference_status", "release_status", "customer_code", "representative_code", "item_code", "emission_from", "emission_to", "limit"],
    },
    { label: "Abrir pedido", method: "GET", path: "/api/sales-order/{code}", fields: [id("code", "Pedido")] },
    { label: "Listar pedidos por situação", method: "GET", path: "/api/sales-order/status/{status}", fields: [{ name: "status", label: "Situação", required: true, options: ["R", "P", "A", "OA", "OF", "F", "CANCELLED"], help: "Rascunho (R), Confirmado (P), Em análise (A), Orçamento (OF/OA), Faturado (F) ou Cancelado." }] },
    { label: "Analisar", method: "POST", path: "/api/sales-order/{code}/analyze", resultPath: "/api/sales-order/{code}", fields: [id("code", "Pedido"), json('{"area":"COMMERCIAL","status":"APPROVED","reason":"Análise concluída"}')] },
    { label: "Atender", method: "POST", path: "/api/sales-order/{code}/attend", resultPath: "/api/sales-order/{code}", fields: [id("code", "Pedido"), json('{"reason":"Pedido atendido","event_date":"2026-07-14"}')] },
    { label: "Conferir", method: "POST", path: "/api/sales-order/{code}/conference", resultPath: "/api/sales-order/{code}", fields: [id("code", "Pedido"), json('{"status":"CONFERRED","reason":"Conferência concluída"}')] },
    { label: "Motivo de atraso", method: "POST", path: "/api/sales-order/{code}/delay-reason", resultPath: "/api/sales-order/{code}", fields: [id("code", "Pedido"), json('{"reason":"Aguardando transportadora","action":"RESCHEDULE"}')] },
  ]),
  VVND0610: routine("VVND0610", "Reajuste de venda recorrente", "Recalcula o valor ajustado de um contrato recorrente existente e registra o motivo comercial do reajuste.", [
    { label: "Consultar recorrência", method: "GET", path: "/api/recurring-sales/{code}", fields: [id("code", "Venda recorrente")] },
    { label: "Listar recorrências", method: "GET", path: "/api/recurring-sales/list" },
    { label: "Recalcular reajuste", method: "POST", path: "/api/recurring-sales/{code}/recalculate-adjustment", fields: [id("code", "Venda recorrente"), { name: "adjustment_percent", label: "Percentual de reajuste", type: "number", required: true, help: "Use o percentual contratual (índice/teto definidos no cadastro). Negativo apenas para redução autorizada." }, { name: "reason", label: "Motivo", type: "textarea", required: true, help: "Justificativa comercial e base legal/contratual do reajuste." }] },
  ]),
  // A tela possui implementação dedicada para o multipart e os downloads; esta
  // definição mantém o contrato catalogado para as auditorias de cobertura.
  VSAC0200: routine("VSAC0200", "Relatórios, etiquetas e anexos do atendimento", "Consulta relatórios e gerencia arquivos reais vinculados ao chamado.", [
  ...
  VREP0600: routine("VREP0600", "Complementos do representante", "Mantém segmentos, planos de venda, interesses e endereços de correspondência do representante.", [
    { label: "Consultar complementos do representante", method: "GET", path: "/api/representatives/{code}", fields: [id("code", "Representante")] },
    { label: "Listar representantes", method: "GET", path: "/api/representatives/list" },
    { label: "Listar planos de venda", method: "GET", path: "/api/representatives/sales-plans" },
    { label: "Listar classificações de interesse", method: "GET", path: "/api/representatives/interest-classifications" },
    { label: "Listar segmentos de mercado", method: "GET", path: "/api/customers/support/market-segments" },
    { label: "Vincular segmento", method: "POST", path: "/api/representatives/segments", fields: [json('{"representative_code":1,"market_segment_code":1,"is_active":true}')] },
  ...
  VEST0400: routine("VEST0400", "Consultas de movimentos e saldos por almoxarifado", "Consulta movimentos, saldo pontual e posição consolidada do estoque real.", [
    ...
    { label: "Consultar saldo", method: "GET", path: "/api/stock/balances/get", fields: [{ name: "item_code", label: "Item", required: true }, { name: "warehouse_id", label: "Almoxarifado", type: "number", required: true }, { name: "lot", label: "Lote" }], query: ["item_code", "warehouse_id", "lot"] },
  ...
  VITE0118: routine("VITE0118", "Regras de itens configurados", "Mantém e avalia regras que preenchem campos do item a partir das respostas do configurador.", [
    list("/api/configurator/items/{itemCode}/rules", [id("itemCode", "Item")], [], "Consultar regras do item"),
    create("/api/configurator/item-rules", '{"item_code":100,"target_table":"items","target_field":"description","content":"Configurado","formula":"","description":"Preencher descrição","situation":"ACTIVE","conditions":[]}', false, "Cadastrar regra do item"),
